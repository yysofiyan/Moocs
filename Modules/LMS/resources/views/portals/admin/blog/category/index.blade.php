...
<x-dashboard-layout>
    <x-slot:title>{{ translate('Manage Blog Category') }}</x-slot:title>
    <!-- BREADCRUMB -->
    <x-portal::admin.breadcrumb title="Blog Category" page-to="Category"
        action-route="{{ route('blog.category.create') }}" />

    <div class="card">
        <div class="flex items-center gap-2 pb-5 mb-5 border-b border-gray-200 dark:border-dark-border">
            <a href="{{ route('blog.category.index', ['filter' => 'all']) }}"
                class="badge badge-primary-outline b-outline group/b-counter is-hover-active rounded-full dk-theme-card-square {{ get_active_filter_tab() === 'all' ? 'active' : '' }}">
                {{ translate('All') }}
                <span class="badge-counter rounded-full dk-theme-card-square">
                    {{ $countData['total'] ?? 0 }}
                </span>
            </a>
            <a href="{{ route('blog.category.index') }}"
                ...
                {{ translate('Published') }}
                <span class="badge-counter rounded-full dk-theme-card-square">{{ $countData['published'] ?? 0 }}</span>
            </a>
            <a href="{{ route('blog.category.index', ['filter' => 'trash']) }}"
        ...
        @if ($categories->count() > 0)
            ...
            <div class="overflow-x-auto scrollbar-table">
                <table class="table-auto w-full whitespace-nowrap text-left font-medium leading-none">
                    <thead class="text-primary-500">
                        <tr>
                            <th
                                class="bg-[#F2F4F9] dark:bg-dark-card-two first:rounded-l-lg last:rounded-r-lg px-3.5 py-4">
                                {{ translate('Name') }}
                            </th>
                            <th
                                class="bg-[#F2F4F9] dark:bg-dark-card-two first:rounded-l-lg last:rounded-r-lg px-3.5 py-4">
                                {{ translate('Total Blog') }}
                            </th>
                            <th
                                class="bg-[#F2F4F9] dark:bg-dark-card-two first:rounded-l-lg last:rounded-r-lg px-3.5 py-4 w-10">
                                {{ translate('Action') }}
                            </th>
                        </tr>
                    </thead>
                    <tbody class="divide-y divide-gray-200 dark:divide-dark-border-three">
                        @foreach ($categories as $category)
                            @php $translations = parse_translation($category); @endphp
                            <tr class="text-gray-500 dark:text-dark-text">
                                <td class="px-3.5 py-4">{{ $translations['name'] ?? ($category->name ?? '') }}</td>
                                <td class="px-3.5 py-4">{{ $category->blogs()->count() }}</td>
                                <td class="px-3.5 py-4">
                                    <div class="flex items-center justify-end gap-1">
                                        ...
                                        @if ($category->trashed())
                                            ...
                                            @endphp
                                            <button data-action="{{ route('blog.category.restore', $category->id) }}"
                                        ...
                                        @else
                                            <a href="{{ route('blog.category.translate', ['id' => $category->id, 'locale' => app()->getLocale()]) }}"
                                        ...
                                        <button data-action="{{ route('blog.category.destroy', $category->id) }}"
            ...
            {{ $categories->links('portal::admin.pagination.paginate') }}
    </div>
@else
    <x-portal::admin.empty-card title="No Category" action="{{ route('blog.category.create') }}" btnText="Add New" />
    @endif
</x-dashboard-layout>
