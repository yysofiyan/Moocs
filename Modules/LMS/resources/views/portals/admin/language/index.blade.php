<x-dashboard-layout>
    ...
    <!-- BREADCRUMB -->
    <x-portal::admin.breadcrumb title="Language" page-to="Language" action-route="{{ route('language.create') }}" />
    <div class="card overflow-hidden">
        <div class="flex items-center gap-2 pb-5 mb-5 border-b border-gray-200 dark:border-dark-border">
            <a href="{{ route('language.index', ['filter' => 'all']) }}"
            ...
            <a href="{{ route('language.index', ['filter' => 'trash']) }}"
                ...
                <table
                    ...
                    <tbody class="divide-y divide-gray-200 dark:divide-dark-border-three">
                        @foreach ($languages as $language)
                            @php
                                $translations = parse_translation($language);
                            @endphp
                            <tr>
                                <td class="px-3.5 py-4">{{ $translations['name'] ?? $language->name }}</td>
                                ...
                                <td class="px-3.5 py-4">
                                    <div class="flex items-center justify-end gap-1">
                                        @if ($language->trashed())
                                            <button
                                                data-action="{{ route('language.restore', ['id' => $language->id]) }}"
                                        ...
                                        @else
                                            <a href="{{ route('language.translate', ['id' => $language->id, 'locale' => app()->getLocale()]) }}"
                                        ...
                                        <button class="btn-icon btn-danger-icon-light size-8 delete-btn-cs"
                                            data-action="{{ route('language.destroy', $language->id) }}">
