...
<x-dashboard-layout>
    <x-slot:title> {{ translate('Manage Category') }} </x-slot:title>
    <!-- BREADCRUMB -->
    <x-portal::admin.breadcrumb title="Category" page-to="Category" action-route="{{ route('category.create') }}" />

    <div class="card overflow-hidden">
        <div class="flex items-center gap-2 pb-5 mb-5 border-b border-gray-200 dark:border-dark-border">
            <a href="{{ route('category.index', ['filter' => 'all']) }}"
            ...
            <a href="{{ route('category.index', ['filter' => 'trash']) }}"
                                    ...
                                    <div class="form-check form-switch">
                                        <label class="inline-flex items-center me-5 cursor-pointer">
                                            <input type="checkbox" class="hidden appearance-none peer status-change"
                                                name="status"
                                                data-action="{{ route('category.status', $category->id) }}"
                                                {{ $category->status == 1 ? 'checked' : '' }}
                                                {{ $category->trashed() ? 'disabled' : '' }}>
                                ...
                                <td class="px-3.5 py-4">
                                    <div class="flex items-center justify-end gap-1">
                                        ...
                                        @if ($category->trashed())
                                            ...
                                            @endphp
                                            <button data-action="{{ route('category.restore', $category->id) }}"
                                        ...
                                        @else
                                            <a href="{{ route('category.translate', ['id' => $category->id, 'locale' => app()->getLocale()]) }}"
                                        ...
                                        <button data-action="{{ route('category.destroy', $category->id) }}"
        ...
        @else
            <x-portal::admin.empty-card title="No Category" action="{{ route('category.create') }}"
                btnText="Add New" />
        @endif
    </div>
</x-dashboard-layout>
