<x-dashboard-layout>
    <x-slot:title>{{ translate('Manage Level') }}</x-slot:title>
    <!-- BREADCRUMB -->
    <x-portal::admin.breadcrumb title="Level" page-to="Level" action-route="{{ route('level.create') }}" />

    <div class="card overflow-hidden">

        <div class="flex items-center gap-2 pb-5 mb-5 border-b border-gray-200 dark:border-dark-border">
            <a href="{{ route('level.index', ['filter' => 'all']) }}"
            ...
            <a href="{{ route('level.index', ['filter' => 'trash']) }}"
                            ...
                            <tr>
                                <td class="px-3.5 py-4">
                                    <h6 class="leading-none text-heading dark:text-white font-semibold">
                                        <a href="#">{{ $translations['name'] ?? ($level->name ?? '') }}</a>
                                    </h6>
                                </td>
                                <td class="px-3.5 py-4">
                                    <div class="flex items-center gap-1">
                                        ...
                                        @if ($level->trashed())
                                            <button data-action="{{ route('level.restore', $level->id) }}"
                                        ...
                                        @else
                                            <a href="{{ route('level.translate', ['id' => $level->id, 'locale' => app()->getLocale()]) }}"
                                        ...
                                        <button class="btn-icon btn-danger-icon-light size-8 delete-btn-cs"
                                            data-action="{{ route('level.destroy', $level->id) }}"
...
@else
    <x-portal::admin.empty-card title="No level" action="{{ route('level.create') }}" btnText="Add New" />
    @endif
</x-dashboard-layout>
