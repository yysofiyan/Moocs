@php
    ...
    $text = translate('If You trash, if have hero, all data will be trash.');
...
<x-dashboard-layout>
    ...
    <!-- BREADCRUMB -->
    <x-portal::admin.breadcrumb title="Hero" page-to="Hero" action-route="{{ route('hero.create') }}" />
    <div class="card overflow-hidden">
        <div class="flex items-center gap-2 pb-5 mb-5 border-b border-gray-200 dark:border-dark-border">
            <a href="{{ route('hero.index', ['filter' => 'all']) }}"
            ...
            <a href="{{ route('hero.index', ['filter' => 'trash']) }}"
        ...
        @if ($heroes->count() > 0)
                    ...
                    <tbody class="divide-y divide-gray-200 dark:divide-dark-border-three">
                        @foreach ($heroes as $hero)
                            <tr>
                                ...
                                <td class="px-3.5 py-4">
                                    @php
                                        $class = $hero->status ? 'badge-primary-outline' : 'badge-danger-outline';
                                        $status = $hero->status ? 'Active' : 'Inactive';
                                    @endphp

                                    <div class="badge {{ $class }} b-outline text-[14px]">
                                    ...
                                    <div class="flex items-center justify-end gap-1">
                                        @if ($hero->trashed())
                                            @php
                                                ...
                                                $text = translate(
                                                    'If You delete it, if have hero, all data will be deleted permanently',
                                                );
                                            @endphp
                                            <button data-action="{{ route('hero.restore', ['id' => $hero->id]) }}"
                                        ...
                                        <button class="btn-icon btn-danger-icon-light size-8 delete-btn-cs"
                                            data-action="{{ route('hero.destroy', $hero->id) }}"
            ...
            <!-- Start Pagination -->
            {{ $heroes->links('portal::admin.pagination.paginate') }}
        @else
            <x-portal::admin.empty-card title="No Hero Created" action="{{ route('hero.create') }}"
                btnText="Add New" />
        @endif
    </div>
