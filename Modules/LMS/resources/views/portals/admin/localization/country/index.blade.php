<x-dashboard-layout>
    <x-slot:title>{{ translate('Country Manage') }}</x-slot:title>
    <!-- BREADCRUMB -->
    <x-portal::admin.breadcrumb title="Country" page-to="Country" action-route="{{ route('country.create') }}" />
    <div class="card overflow-hidden">
        ...
        <div class="overflow-x-auto" id="data-content">
            <div class="flex items-center gap-2 pb-5 mb-5 border-b border-gray-200 dark:border-dark-border">
                <a href="{{ route('country.index', ['filter' => 'all']) }}"
                ...
                <a href="{{ route('country.index', ['filter' => 'trash']) }}"
                ...
                <table
                    ...
                    <tbody class="divide-y divide-gray-200 dark:divide-dark-border-three">
                        ...
                        @foreach ($countries as $country)
                            @php $translations = parse_translation($country); @endphp
                            <tr>
                                <td class="px-3.5 py-4">
                                    {{ $translations['name'] ?? $country->name }}
                                </td>
                                ...
                                <td class="px-3.5 py-4">
                                    <div class="flex items-center justify-end gap-1">
                                        <a href="{{ route('country.translate', ['id' => $country->id, 'locale' => app()->getLocale()]) }}"
                                        ...
                                        @php
                                            ...
                                            $text = translate('If You trash it,State and city data will be trashed.');
                                            if ($country->trashed()) {
                                                $title = translate('Are you sure you want to delete this permanently');
                                                $text = translate(
                                                    'If You delete it, State and city data will be deleted permanently.',
                                                );
                                            }
                                        @endphp
                                        @if ($country->trashed())
                                            <button
                                                data-action="{{ route('country.restore', ['id' => $country->id]) }}"
                                        ...
                                        <button class="btn-icon btn-danger-icon-light size-8 delete-btn-cs"
                                            data-action="{{ route('country.destroy', $country->id) }}"
        ...
        {{ $countries->links('portal::admin.pagination.paginate') }}
    @else
        <x-portal::admin.empty-card title="No country"
            action="{{ $action = route('country.create') }}" btnText="Add New" />
        @endif
    </div>
</x-dashboard-layout>
