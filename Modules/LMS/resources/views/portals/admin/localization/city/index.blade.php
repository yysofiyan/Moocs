<x-dashboard-layout>
    <x-slot:title>{{ translate('Manage City') }}</x-slot:title>
    <!-- BREADCRUMB -->
    <x-portal::admin.breadcrumb title="City" page-to="City" action-route="{{ route('city.create') }}" />
    <div class="card overflow-hidden">
        ...
        <div class="overflow-x-auto" id="data-content">
            <div class="flex items-center gap-2 pb-5 mb-5 border-b border-gray-200 dark:border-dark-border">
                <a href="{{ route('city.index', ['filter' => 'all']) }}"
                ...
                <a href="{{ route('city.index', ['filter' => 'trash']) }}"
                ...
                <table
                    ...
                    <tbody class="divide-y divide-gray-200 dark:divide-dark-border-three">
                        @foreach ($cities as $city)
                            @php 
                                $translations = parse_translation($city);
                                $countryTranslations = [];
                                $stateTranslations = [];

                                if (method_exists($city, 'country')) {
                                    $countryTranslations = parse_translation($city->country); 
                                }

                                if (method_exists($city, 'state')) {
                                    $stateTranslations = parse_translation($city->state); 
                                }
                            @endphp
                            <tr>
                                <td class="px-3.5 py-4">
                                    {{ $translations['name'] ?? $city->name ?? '' }}
                                </td>
                                <td class="items-center gap-2 px-3.5 py-4">
                                    {{ $stateTranslations['name'] ?? $city->state?->name ?? '' }}
                                </td>
                                <td class="items-center gap-2 px-3.5 py-4">
                                    {{ $countryTranslations['name'] ?? $city->country?->name ?? '' }}
                                </td>
                                <td class="items-center gap-2 px-3.5 py-4">
                                    ...
                                    <div class="form-check form-switch">
                                        <label class="inline-flex items-center me-5 cursor-pointer">
                                            <input type="checkbox" class="hidden appearance-none peer status-change"
                                                name="status" data-action="{{ route('city.status', $city->id) }}"
                                ...
                                <td class="px-3.5 py-4">
                                    <div class="flex items-center justify-end gap-1">
                                        <a href="{{ route('city.translate', ['id' => $city->id, 'locale' => app()->getLocale()]) }}"
                                        ...
                                        @if ($city->trashed())
                                            <button data-action="{{ route('city.restore', $city->id) }}"
                                        ...
                                        <button data-action="{{ route('city.destroy', $city->id) }}"
        ...
        <!-- Start Pagination -->
        {{ $cities->links('portal::admin.pagination.paginate') }}
    @else
        <x-portal::admin.empty-card title="No city" action="{{ route('city.create') }}"
            btnText="Add New" />
        @endif
    </div>
</x-dashboard-layout>
